...
def exact_feature_distribution_matching(content_feat, style_feat):
    ...
    index_content = index_content.cpu().numpy()
    inverse_index = index_content.argsort(-1)
    inverse_index = torch.tensor(inverse_index).cuda()
...
def calc_style_loss(input, target):
    ...
    inverse_index = index_content.argsort(-1)
    inverse_index = torch.tensor(inverse_index).cuda()
    return nn.MSELoss()(input.view(B,C,-1), value_style.gather(-1, inverse_index))
...
#
# Smalldecoder  = nn.Sequential(
#               nn.ReflectionPad2d((1, 1, 1, 1)),
#               nn.Conv2d(128, 64, 3, 1, 0),  # Conv4-1
#               nn.ReLU(inplace=True),
#               nn.UpsamplingNearest2d(scale_factor=2),
#               nn.ReflectionPad2d((1, 1, 1, 1)),
#               nn.Conv2d(64, 64, 3, 1, 0, dilation=1),  # Conv3-4
#               nn.ReLU(inplace=True),
#               nn.ReflectionPad2d((1, 1, 1, 1)),
#               nn.Conv2d(64, 64, 3, 1, 0, dilation=1),  # Conv3-3
#               nn.ReLU(inplace=True),
#               nn.ReflectionPad2d((1, 1, 1, 1)),
#               nn.Conv2d(64, 64, 3, 1, 0, dilation=1),  # Conv3-2
#               nn.ReLU(inplace=True),
#               nn.ReflectionPad2d((1, 1, 1, 1)),
#               nn.Conv2d(64, 32, 3, 1, 0, dilation=1),  # Conv3-1
#               nn.UpsamplingNearest2d(scale_factor=2),
#               nn.ReflectionPad2d((1, 1, 1, 1)),
#               nn.Conv2d(32, 32, 3, 1, 0, dilation=1),  # Conv2-2
#               nn.ReLU(inplace=True),
#               nn.ReflectionPad2d((1, 1, 1, 1)),
#               nn.Conv2d(32, 16, 3, 1, 0, dilation=1),  # Conv2-1
#               nn.ReLU(inplace=True),
#               nn.UpsamplingNearest2d(scale_factor=2),
#               nn.ReflectionPad2d((1, 1, 1, 1)),
#               nn.Conv2d(16, 16, 3, 1, 0, dilation=1),  # Conv1-2
#               nn.ReLU(inplace=True),
#               nn.ReflectionPad2d((1, 1, 1, 1)),
#               nn.Conv2d(16, 3, 3, 1, 0, dilation=1),  # Conv1-1
#               nn.ReLU(inplace=True),
# )
# # bridge the dimension mismatch using a 1x1 linear layer
#
# SmallEncoder = nn.Sequential(
#             nn.Conv2d(3, 3, 1, 1, 0),   #Conv0
#             nn.ReflectionPad2d((1,1,1,1)),
#             nn.Conv2d(3, 16, 3, 1, 0, dilation=1), #Conv11
#             nn.ReLU(inplace=True), # relu1-1
#             nn.ReflectionPad2d((1, 1, 1, 1)),
#             nn.Conv2d(16, 16, 3, 1, 0, dilation=1),     #Conv12
#             nn.ReLU(inplace=True),  # relu1-2
#             nn.MaxPool2d(kernel_size=2,stride=2,return_indices=False),
#             nn.ReflectionPad2d((1,1,1,1)),
#             nn.Conv2d(16, 32, 3, 1, 0, dilation=1),   #Conv21
#             nn.ReLU(inplace=True),    # relu2-1
#             nn.ReflectionPad2d((1,1,1,1)),
#             nn.Conv2d(32, 32, 3, 1, 0, dilation=1),   #Conv22
#             nn.ReLU(inplace=True), # relu2-2
#             nn.MaxPool2d(kernel_size=2,stride=2,return_indices=False),
#             nn.ReflectionPad2d((1,1,1,1)),
#             nn.Conv2d(32, 64, 3, 1, 0, dilation=1),   #Conv31
#             nn.ReLU(inplace=True),    # relu3-1
#             nn.ReflectionPad2d((1,1,1,1)),
#             nn.Conv2d(64, 64, 3, 1, 0, dilation=1),   #Conv32
#             nn.ReLU(inplace=True),    # relu3-2
#             nn.ReflectionPad2d((1,1,1,1)),
#             nn.Conv2d(64, 64, 3, 1, 0, dilation=1),   #Conv33
#             nn.ReLU(inplace=True),    # relu3-3
#             nn.ReflectionPad2d((1,1,1,1)),
#             nn.Conv2d(64, 64, 3, 1, 0, dilation=1),   #Conv34
#             nn.ReLU(inplace=True),    # relu3-4
#             nn.MaxPool2d(kernel_size=2,stride=2,return_indices=False),
#             nn.ReflectionPad2d((1,1,1,1)),
#             nn.Conv2d(64, 128, 3, 1, 0),               #Conv41
#             nn.ReLU(inplace=True),     # relu4-1, this is the last layer used
#             # nn.ReflectionPad2d((1,1,1,1)),
#             # nn.Conv2d(128, 128, 3, 1, 0),               #Conv42
#             # nn.ReLU(inplace=True),    # relu4-2
#             # nn.ReflectionPad2d((1,1,1,1)),
#             # nn.Conv2d(128, 128, 3, 1, 0),               #Conv43
#             # nn.ReLU(inplace=True),    # relu4-3
#             # nn.ReflectionPad2d((1,1,1,1)),
#             # nn.Conv2d(128, 128, 3, 1, 0),               #Conv44
#             # nn.ReLU(inplace=True),    # relu4-4
#             # nn.MaxPool2d(kernel_size=2,stride=2,return_indices=False),
#             # nn.ReflectionPad2d((1,1,1,1)),
#             # nn.Conv2d(128, 128, 3, 1, 0),
#             # nn.ReLU(inplace=True),                  # Conv51
# )
...
class SmallEncoder(nn.Module):
    ...
    #     out41_aux = self.relu(self.conv41_aux(out41))
    #     # out51_aux = self.relu(self.conv51_aux(out51))
    #     return out11_aux, out21_aux, out31_aux, out41_aux, out41  # output out41
    #
    ...
    #     else:
    #         out41_aux = self.conv41_aux(out41)
    #     return out11, out21, out31, out41, out41_aux
